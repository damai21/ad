...
public final class Banner implements Cloneable {
	...
	public void removeFormatEntry(Object entry) {
		ArrayList list = new ArrayList();
		list.addAll(Arrays.asList(format));
		list.remove(entry);
		format = list.toArray();
	}

	public int getFormatListSize() {
		synchronized (format) {
	...
	public static class BannerSize implements Comparable<BannerSize> {
		...
		public boolean equals(Object o) {
			if (o instanceof  Banner.BannerSize) {
				return ((Banner.BannerSize)o).h == h && ((Banner.BannerSize)o).w == w;
			} else {
				return false;
			}
		}
