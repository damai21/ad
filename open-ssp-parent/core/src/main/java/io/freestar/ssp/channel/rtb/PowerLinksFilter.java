...
public class PowerLinksFilter extends DemandBrokerFilter {
    private static final Logger LOG = LoggerFactory.getLogger(PowerLinksFilter.class);
    ...
    public String filterRequest(Gson gson, BidRequest bidrequest) {
        LOG.info(bidrequest.hashCode()+" A:filter: "+bidrequest.getAllImp());
        try {
            ...
            site.remove("name");
//        site.remove("domain");
            JsonArray siteCat = (JsonArray) site.get("cat");
            if (siteCat.size() == 0) {
              site.remove("cat");
            }
            site.remove("pagecat");
            ...
            JsonObject publisher = (JsonObject) site.get("publisher");
            publisher.remove("domain");
            publisher.remove("Ext");
