...
//                .setExtension()
	...
	private Object ext;
        ...
        for (ParamValue pOrigin : pValueList) {
            ...
            i.setImpid(pValues.getAdUnitCode());
            ...
            if (overrideBidFloor != null) {
                ...
                try {
                    ...
                    log.debug("price floor does not exist for site(1): "+site.getId());
                ...
                try {
                    ...
                    log.debug("price floor does not exist for site(2): "+site.getId());
                    i.setBidfloor(0f);
                    i.setBidfloorcur(CurrencyCache.instance.getBaseCurrency());
                }
            }
            i.setSecure(ImpressionSecurity.SECURE);
            bidRequest.addImp(i);

        }

        if (bidRequest.getAllImp().size() == 0) {
 ...
 //               .setBuyeruid()
    ...
    private Geo createSiteGeo(HeaderBiddingParamValue pValues) {
        ...
        try {
            ...
            log.debug("missing geo code properties: "+pValues.getFsLoc());
        }
        ...
        if (ipAddress != null && !ipAddress.equalsIgnoreCase("localhost") && !ipAddress.equalsIgnoreCase("0:0:0:0:0:0:0:1") && !ipAddress.equalsIgnoreCase("127.0.0.1")) {
            try {
                ...
                log.warn("could not obtain geo code (1): "+e.getMessage(), e);
                // no new address offerings to help
                // keep what we have and move on
            } catch (UnavailableHandlerException e) {
                log.warn("could not obtain geo code (2): "+e.getMessage());
                // no new address offerings to help
                // keep what we have and move on
    ...
    private Banner createBanner(HeaderBiddingParamValue pValues) throws RequestException {
        log.warn("Header Request parameters : \n" + new Gson().toJson(pValues+"\n\n"));
        ...
        try {
            ...
            log.error("Missing or malformed banner size: "+pValues.getSite()+":"+pValues.getAdUnitCode()+"::"+pValues.getSize());
            throw new RequestException(ERROR_CODE.E906, "Missing or malformed banner size");
        }
