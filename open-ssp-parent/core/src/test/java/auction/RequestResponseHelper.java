...
public class RequestResponseHelper {
	...
	public static BidRequest.Builder createRequest(
			final String id,
			final float floor,
			final float dealFloor,
			final String cur,
			final String dealid,
			final int private_auction) {
	...
	public static BidResponse createResponse(
	        final String id,
            final float price,
            final String cur,
            final String dealId,
	...
	public static BidResponse createResponseMultiBidMultiSeat(
	        final float[][] prices,
            final String cur,
            final String dealId,
            final String adId,
		...
		for (int i = 0; i < prices.length; i++) {
			...
			for (int j = 0; j < prices[i].length; j++) {
				bids[j] = createBid(prices[i][j], dealId, adId, cId, crId);
			}
			// SeatBid
	...
	public static BidResponse createResponseMultiBid(
	        final float[] prices,
            final String cur,
            final String dealId,
            final String adId,
		...
		for (int i = 0; i < prices.length; i++) {
			bids[i] = createBid(prices[i], dealId, adId, cId, crId);
		}
		final SeatBid[] seats = new SeatBid[1];
		// SeatBid
		seats[0] = createSeatBid(bids);
