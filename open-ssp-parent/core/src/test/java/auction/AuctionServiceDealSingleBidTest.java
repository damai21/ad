...
public class AuctionServiceDealSingleBidTest {
	...
	public final void testSingleDealBidWins() {
		...
		String id = "ssp-req-id";
		final BidRequest bidRequest = createRequest(id, impFloor, dealFloor, currency, true).build();

		// bidresponse
		final float bidPrice = 3.5f;
		final BidResponse response = createResponse(id, bidPrice, currency, "998877", "5-5", "c-1", "cr-1");
	...
	private BidRequest.Builder createRequest(final String id, final float floor, final float dealFloor, final String cur, final boolean withPMP) {
	...
	private static BidResponse createResponse(
		...
		bids[0] = createBid(price, dealId, adId, cId, crId);
