...
public class JsonPostConnector extends DefaultConnector {
	...
	public String connect(final StringEntity entity, HttpPost httpPost) throws BidProcessingException {
		...
		try {
            ...
            log.info("calling: "+httpPost.getURI().toASCIIString());
            httpResponse = httpClient.execute(httpPost);
			final int statusCode = httpResponse.getStatusLine().getStatusCode();
            log.info(httpPost.getURI().getHost()+" - status: "+statusCode);
            ...
            System.err.println("["+httpPost.getURI().toASCIIString()+"] "+e.getMessage());
		    e.printStackTrace();
