...
import io.freestar.ssp.server.common.core.system.job.GeoIp2TransferJob;
import io.freestar.ssp.server.common.core.system.job.LogArchiveTransferJob;
...
public class ApplicationInit extends GenericServlet {
	...
	public void init() {
		...
		// initing Geo Data Transfer job
		// every 30 minutes on every day
		if (!GeoIp2TransferJob.DATABASE.exists()) {
//			GeoIp2TransferJob.initDatabase();
		}
		JobService.instance.initJob(JobConfig.newBuilder().setJobClass(GeoIp2TransferJob.class).setJobName("GeoIp2TransferJob").setExpression("0 0/30 * * * ?").build());

		// initing Log Archive Transfer job
		// every 5 minutes on every day
		JobService.instance.initJob(JobConfig.newBuilder().setJobClass(LogArchiveTransferJob.class).setJobName("LogArchiveTransferJob").setExpression("0 0/5 * * * ?").build());
